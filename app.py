...
def generate_pdf_report(review_output, template_content):
    ...
    pdf.cell(0, 10, "CTD Module 3 Review Report", ln=True, align='C')
    pdf.ln(5)
    ...
    pdf_path = "CTD_Module3_Review_Report.pdf"
    pdf.output(pdf_path)
    return pdf_path
...
st.markdown(
    """
    Upload the Module 3 CTD document (PDF format) and a text file containing your desired review format template.
    ...
    The AI agent will analyze the entire document and generate a detailed, structured review following your template.
...
uploaded_pdf = st.file_uploader("Upload Module 3 (PDF format only)", type=["pdf"], key="module3_pdf")
...
if uploaded_pdf and uploaded_template:
    ...
    if not template_content:
        ...
        if document_text:
            ...
            if st.button("Analyze Module 3"):
                with st.spinner("Analyzing the document using the AI agent..."):
                    review_output = review_ctd_document(document_text, template_content)
                ...
                if review_output:
                    ...
                    with open(pdf_path, "rb") as f:
                        st.download_button("Download Review Report (PDF)", f, file_name="CTD_Module3_Review_Report.pdf", mime="application/pdf")
                else:
                    st.error("Failed to generate a review. Please try again.")
else:
    st.info("Please upload both the Module 3 PDF and the review format template to proceed.")
